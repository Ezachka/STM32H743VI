...
bool SoftUartGpioReadPin(GPIO_TypeDef *GPIOx, uint16_t GPIO_Pin)
{
	//return HAL_GPIO_ReadPin(GPIOx,GPIO_Pin);
    	return GPIOx->IDR & (0x1UL<<GPIO_Pin);
}

// Write TX single Pin Value
void SoftUartGpioWritePin(GPIO_TypeDef *GPIOx, uint16_t GPIO_Pin, bool PinState)
{
    if (PinState)
    {
        GPIOx->ODR |= (0x1UL << GPIO_Pin);     
    }
    else
    {
        GPIOx->ODR &= ~(0x1UL << GPIO_Pin) ;  
...
{
	SoftUartGpioWritePin(SU->TxPort,SU->TxPin,Bit0_1);
}
...
{
    //Call Every (0.2)*(1/460800) = 0,43403 uS SystemCoreClock=480000000
    
    __disable_irq();
    while(SUart[SoftUartNumber].TxNComplated)
    {
        SoftUartHandler();
        USART_SW_TX2_PORT->ODR ^= USART_SW_TX2_PIN;
        delay_sysclk(tick);
    }
    __enable_irq();
...
void soft_usart_with_dwt_delay_boud(uint8_t SoftUartNumber,uint32_t baudrate)
{
    ...
    float every_call = 0.2f * (1.0f / (float)baudrate);  //s 1/SystemCoreClock =s
    
    __disable_irq();
    while(SUart[SoftUartNumber].TxNComplated)
    {
        SoftUartHandler();
        //sUSART_SW_TX2_PORT->ODR ^= USART_SW_TX2_PIN;
        delay_sysclk((uint32_t)(every_call * (float)SystemCoreClock));
    }
    __enable_irq();
