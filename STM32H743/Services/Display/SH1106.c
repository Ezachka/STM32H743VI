...
//static uint8_t SH1106_Buffer[(128 + (64 / 8) * SH1106_WIDTH) % (SH1106_WIDTH / 8)] = 1 << (64 % 8); 
...
uint8_t SH1106_Init(void) {
	...
	//if (HAL_I2C_IsDeviceReady(SH1106_I2C, SH1106_I2C_ADDR, 1, 20000) != HAL_OK) {
     //   if (   I2C_IsDeviceConnected( I2C1, SH1106_I2C_ADDR ) != true){
		/* Return false */
	//	return 0;
	//}
...
void SH1106_Putf(float value,uint8_t after_piont, FontDef_t* Font, SH1106_COLOR_t color) {
    char buffer[20];  // Буфер для строки
    switch(after_piont)
    {
        case 0:
        sprintf(buffer, "%.0f",  (double)value);  // Преобразование float в строку с 0 знаками после запятой
        break;
        case 1:
        sprintf(buffer, "%.1f",  (double)value);  // Преобразование float в строку с 1 знаками после запятой
        break;   
        case 2:
        sprintf(buffer, "%.2f",  (double)value);  // Преобразование float в строку с 2 знаками после запятой
        break;
        case 3:
        sprintf(buffer, "%.3f",  (double)value);  // Преобразование float в строку с 3 знаками после запятой
        break;
        case 4:
        sprintf(buffer, "%.4f",  (double)value);  // Преобразование float в строку с 4 знаками после запятой
        break;
        case 5:
        sprintf(buffer, "%.5f",  (double)value);  // Преобразование float в строку с 5 знаками после запятой
        break;
        default:
        sprintf(buffer, "%.3f",  (double)value);  // Преобразование float в строку с 3 знаками после запятой
        break; 
    }
    SH1106_Puts(buffer, Font, color);  // Вывод строки на дисплей
}
void SH1106_PutHex(uint32_t value, FontDef_t* Font, SH1106_COLOR_t color) {
    char buffer[11];  // Буфер для строки "0x11223344" (10 символов + терминальный '\0')
    sprintf(buffer, "0x%08X", value);  // Преобразование uint32_t в строку в формате 0x11223344
    SH1106_Puts(buffer, Font, color);  // Вывод строки на дисплей
}
...
for(i = 0; i < count; i++)
dt[i+1] = data[i];
 i2c_transmit(I2C1,address, dt, count+1, SH1106_I2C_TIMEOUT); 
}
...
void SH1106_I2C_Write(uint8_t address, uint8_t reg, uint8_t data) {
	...
	dt[0] = reg;
	dt[1] = data;
   if(i2c_transmit(I2C1,address, dt, 2, SH1106_I2C_TIMEOUT) != transmitted)
   {     
       disable_counter++;
   }
   if (disable_counter >100)
   {
       //display_off();
       disable_counter=0x00;
   }
}
